const shared_utils = ((() => {
  ...
  const set_random_seed = given_seed => {
   ...
   */
  const get_random_in_range_inclusive_float = (min, max) => // return Math.random() * (max - min) + min;
   ...
   */
  const get_random_in_range_inclusive_int = (min, max) => // return Math.floor(Math.random() * (max - min + 1)) + min;
    Math.floor(local_random() * (max - min + 1)) + min

  // ---

  const release_all_prop_from_object = given_object => {    // purge memory for object reuse
  ...
  const convert_32_bit_float_into_unsigned_16_bit_int_lossy = input_32_bit_buffer => {
  ...
  const convert_16_bit_unsigned_int_to_32_bit_float = input_16_bit_int_buffer => {
  ...
  const conv_bit_size = (input_unsigned_int, bits_per_sample) => { // converts unsigned into signed
  ...
  const convert_16_bit_signed_int_to_32_bit_float = input_8_bit_int_buffer => {
  ...
  const convert_32_bit_float_into_signed_16_bit_int_lossy = input_32_bit_buffer => {
  ...
  const show_object = (given_obj, given_label, given_mode, limit_size_buffer) => {
  ...
  const diff_entire_buffers = (left_obj, right_obj, size_buffer, given_spec) => {
  ...
  const diff_buffers = (left_obj, right_obj, given_spec) => {
